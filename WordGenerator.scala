...
case class WordGenerator(
  ...
  */
  override def doWorkload(df: Option[DataFrame] = None, spark: SparkSession): DataFrame = {
    ...
    val result = "/tmp/test.sh " + numRows + " <end>"!!
    ...
    */
    ...
    val (saveTime, _) = time { writeToDisk("/tmp/one-word-over-and-over.csv", "overwrite", dataDF, spark) }

    val totalTime = generateTime + convertTime
